...
function VehicleAvatar({ photoPath, name }: { photoPath?: string; name: string }) {
  ...
  return (
    <div className="w-16 h-16 rounded-full bg-slate-700 flex items-center justify-center flex-shrink-0 border-2 border-slate-600">
...
export default function VehicleCard({ summary }: VehicleCardProps) {
  ...
  return (
    <div className="bg-card border border-slate-700 rounded-xl p-4 hover:border-slate-600 transition-colors">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 flex-wrap">
            <div>
              <Link
                ...
                className="text-slate-100 font-bold text-lg hover:text-accent transition-colors leading-tight"
              >
                {vehicle.name}
              </Link>
              ...
              {vehicle.license_plate && (
                <p className="text-slate-500 text-xs font-mono mt-0.5">{vehicle.license_plate}</p>
              )}
            </div>
            <div className="text-right flex-shrink-0">
              ...
              <p className="text-slate-500 text-xs">Kosten YTD</p>
            </div>
          </div>
          ...
          {summary.next_service && (
            <div className="mt-2 text-xs text-slate-400">
              <span className="text-slate-500">Nächster Service: </span>
              <span className="text-slate-300">{summary.next_service.task_name}</span>
