...
function VehicleHistory({ vehicleId }: { vehicleId: number }) {
  ...
  return (
    <div className="mt-6">
      <p className="text-slate-400 text-xs uppercase tracking-wider mb-2">Letzte Meldungen</p>
      <div className="flex flex-col gap-1">
        {history.slice(0, 8).map((h) => (
          <div key={h.id} className="flex items-center justify-between text-sm py-1.5 border-b border-slate-700/40 last:border-0">
            <span className="text-accent font-mono">{h.km.toLocaleString('de-DE')} km</span>
            <div className="text-right">
              <span className="text-slate-400 text-xs">{formatDate(h.reported_at)}</span>
  ...
  if (step === 'done') {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6 text-center">
        ...
        <div>
          <p className="text-slate-100 text-xl font-bold">{savedKm.toLocaleString('de-DE')} km gespeichert</p>
  ...
  if (step === 'input' && selected) {
    ...
    return (
      <div className="max-w-sm mx-auto mt-4">
        ...
        <label className="block text-slate-300 text-sm font-medium mt-4 mb-2">
          Notiz <span className="text-slate-500">(optional)</span>
        </label>
        <input
          type="text"
          value={note}
          ...
          className="w-full bg-surface border border-slate-600 focus:border-accent rounded-xl px-4 py-3 text-sm text-slate-100 focus:outline-none"
        />
